...
class TrieNode {
 public:
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    ...
    try {
      return &(children_.emplace(key_char, std::move(child)).first->second);
  ...
  void RemoveChildNode(char key_char) {
    if (HasChild(key_char)) {
      children_.at(key_char).~unique_ptr();
...
class Trie {
 private:
  ...
  bool RemoveI(const std::string &key, std::size_t p, std::unique_ptr<TrieNode> *fa) {
    char ch = key.at(p);
    std::unique_ptr<TrieNode> *cur_p = fa->get()->GetChildNode(ch);
    if (cur_p == nullptr) {
      return false;
    }
    if (p == key.length() - 1) {
      // last pos
      if (cur_p->get()->IsEndNode()) {
        if (cur_p->get()->HasChildren()) {
          // convert cur_p to TrieNode
          auto tmp = new TrieNode(std::move(*(cur_p->get())));
          tmp->SetEndNode(false);
          cur_p->reset(tmp);
          return true;
        }
        // just remove the node
        fa->get()->RemoveChildNode(ch);
        return true;
      }
      return false;
    }
    bool ret = RemoveI(key, p + 1, cur_p);
    if (ret && !cur_p->get()->HasChildren() && !cur_p->get()->IsEndNode()) {
      fa->get()->RemoveChildNode(ch);
    }
    return ret;
  }
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    std::unique_ptr<TrieNode> *fa = &root_;
    for (auto it = key.begin(); it != key.end(); it++) {
      char ch = *it;
      if (fa->get()->HasChild(ch)) {
        std::unique_ptr<TrieNode> *son = fa->get()->GetChildNode(ch);
        if (it == key.end() - 1) {
          if (son->get()->IsEndNode()) {
            latch_.WUnlock();
            return false;
          }
          auto tmp = std::make_unique<TrieNodeWithValue<T>>(std::move(*(son->get())), value);
          fa->get()->RemoveChildNode(ch);
          fa->get()->InsertChildNode(ch, std::move(tmp));
          latch_.WUnlock();
          return true;
        }
        // continue to search down
        fa = fa->get()->GetChildNode(ch);
      } else {
        if (it == key.end() - 1) {
          auto tmp = std::make_unique<TrieNodeWithValue<T>>(ch, value);
          auto ret = fa->get()->InsertChildNode(ch, std::move(tmp));
          latch_.WUnlock();
          return ret != nullptr;
        }
        std::unique_ptr<TrieNode> tmp{new TrieNode(ch)};
        fa = fa->get()->InsertChildNode(ch, std::move(tmp));
      }
    }
    latch_.WUnlock();
    return false;
  ...
  bool Remove(const std::string &key) {
    // we first need to recursively remove down to the end
    latch_.WLock();
    bool ret = RemoveI(key, 0, &root_);
    latch_.WUnlock();
    return ret;
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    latch_.RLock();
    *success = false;
    std::unique_ptr<TrieNode> *cur_p = &root_;
    for (auto it = key.begin(); it != key.end(); it++) {
      char ch = *it;
      cur_p = cur_p->get()->GetChildNode(ch);
      if (cur_p == nullptr) {
        *success = false;
        latch_.RUnlock();
        return {};
      }
      if (it == key.end() - 1 && cur_p->get()->IsEndNode()) {
        auto tmp = dynamic_cast<TrieNodeWithValue<T> *>(cur_p->get());
        if (tmp == nullptr) {
          latch_.RUnlock();
          *success = false;
          return {};
